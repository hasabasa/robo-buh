...
## Сделано (сверх скелета)

- **Движки**: 910/300/200/100/пеня. **XML-генераторы всех форм**: `xml_910/200/300/100` (+сервисы,
  `POST /api/tax/{форма}/{decl}/xml`, локальный ФЛК) — ПРОВИЗОРНЫ (семантические коды, не
  `field_*_NNN`; ждём офиц. машинный шаблон НК-2026).
- **Приём дохода**: MT940/1С + **PDF-выписка Kaspi** (`kaspi_bank_pdf`, детерминированно, локально) +
  **ЭСФ** (`esf_parser` + `esf/soap` — коннектор доказан на проде). КНП-классификатор дополнен на
  реальной выписке (190=доход, 390=перевод).
- **Дашборд** `GET /api/dashboard/{id}` «сколько должен + что не сходится» + красивый фронт Streamlit.

## Открытые задачи

- P0 (внешние блокеры): офиц. XSD форм под НК-2026 (ждём КГД) — привязать провизорные коды к `field_*`.
- P1: **UI «Доходы»** (загрузка выписки из кабинета → сверка → книга) + **подпись декларации через
  NCALayer** (сам NCALayer уже доказан на ЭСФ) = демо «выписка → подписанная 910».
- P1: REST-клиент КГД Тир-1 (`X-Portal-Token`) — гейт онбординга; **тройная сверка 200.00**
  (расчёт ↔ платежи в выписке КНП 010/012/121/122/332/911 ↔ лицевой счёт КГД Тир-2).
- Оргтрек (владелец): заявки Smart Bridge от ТОО "CS", статич. публичный KZ-IP под VPN.
- Позже: модуль долгов (Тир-2), CSV/XLSX-парсеры банков, OCR-путь для не-Kaspi PDF/сканов.
