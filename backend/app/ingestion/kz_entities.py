...
# КНП — код назначения платежа, 3 цифры; метка кириллицей (КНП) или латиницей (KNP),
# разделители пробел/двоеточие/слэш (в MT940 часто /KNP/710/)
RE_KNP = re.compile(r"(?:КНП|KNP)[\s:/]*?(\d{3})\b", re.IGNORECASE)
# КБе — код бенефициара, 2 цифры у метки (КБе / KBE)
RE_KBE = re.compile(r"(?:КБе|KBE)[\s:/]*?(\d{2})\b", re.IGNORECASE)
