...
class TaxYearKZ:
    ...
    # VERIFY: ставки ИПН у источника, вычеты, ООСМС работодателя, СО за работников —
    # зафиксировать по официальным правилам заполнения 200.00 v2026 при реализации
    # зарплатного движка (Фаза 2). Намеренно НЕ заполнено, чтобы не зашить разнобой
    # из статей: значения добавляются сюда только с первоисточником.
